...
NodeLocalEvent(Ptr<Node> node, std::string nodeName)
{
    ...
    if (clock)
    {
        ...
        NS_LOG_INFO(Simulator::Now().GetSeconds() << "s - " 
                    << nodeName << " local event. Lamport Time: " << t);
    }
}
...
ReceiveMessageEvent(Ptr<Node> receiver, std::string receiverName, uint64_t msgTime)
{
    ...
    if (clock)
    {
        ...
        NS_LOG_INFO(Simulator::Now().GetSeconds() << "s - " 
                    << receiverName << " received message (time: " << msgTime 
                    << "). Lamport Time updated to: " << t);
    }
}

void 
SendMessageEvent(Ptr<Node> sender, Ptr<Node> receiver, std::string senderName, std::string receiverName, Time propagationDelay)
{
    ...
    if (senderClock)
    {
        ...
        Simulator::Schedule(propagationDelay, &ReceiveMessageEvent, receiver, receiverName, sendTime);
    }
}
...
main(int argc, char* argv[])
{
    ...
    Simulator::Schedule(Seconds(3.0), &SendMessageEvent, nodeA, nodeB, "Node A", "Node B", Seconds(0.1));
    Simulator::Schedule(Seconds(4.0), &SendMessageEvent, nodeB, nodeA, "Node B", "Node A", Seconds(0.1));
