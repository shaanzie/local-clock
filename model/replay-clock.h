...
class ReplayClock : public LocalClock
{
  public:
     ...
     */
    void SetLocalClock(Time ptime) override;
    ...
    uint64_t GetOffsets() const
    {
        return m_offsets;
    }

    /** @return The packed counter array. */
    uint64_t GetCounters() const
    {
        return m_counters;
    }

    /** @return The number of nodes currently tracked. */
    uint32_t GetNumStoredOffsets() const
    {
     ...
     */
    void SetOffsets(uint64_t offsets)
    {
        m_offsets = offsets;
    }
